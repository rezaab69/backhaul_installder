...
input_ports_interactive() {
    echo "" >&2
    echo -e "  ${BOLD}Port Forwarding Rules:${NC}" >&2
    echo -e "  ${DIM}Formats supported:${NC}" >&2
    echo -e "  ${DIM}  443          → forward port 443 to remote 443${NC}" >&2
    echo -e "  ${DIM}  4000=5000    → local 4000 → remote 5000${NC}" >&2
    echo -e "  ${DIM}  443=1.1.1.1:5201 → local 443 → 1.1.1.1:5201${NC}" >&2
    echo -e "  ${DIM}  443-600      → port range 443 to 600${NC}" >&2
    echo "" >&2
    echo -e "  ${DIM}Enter ports one per line. Empty line when done.${NC}" >&2
    echo "" >&2
...
create_server_config() {
    ...
    if confirm "Configure advanced options?"; then
        ...
        echo -e "${YELLOW}  Log level [info/debug/warn/error]: ${NC}" >&2
        read -rp "  " ll
        log_level="${ll:-info}"
    fi
    ...
    if [[ "$transport" == "wss" || "$transport" == "wssmux" ]]; then
        echo "" >&2
        warn "TLS certificate and key are required for ${transport}." >&2
        echo -e "  ${DIM}Options:${NC}" >&2
        echo -e "  ${CYAN}1)${NC} Generate self-signed certificate" >&2
        echo -e "  ${CYAN}2)${NC} Provide existing paths" >&2
...
mux_streambuffer = 65536"
    ...
    echo "" >&2
    ok "Server config saved: ${config_file}" >&2
    echo "" >&2
    info "Share these details with the client:" >&2
    echo -e "  ${CYAN}Remote addr : ${WHITE}${bind_addr}${NC}" >&2
    echo -e "  ${CYAN}Transport   : ${WHITE}${transport}${NC}" >&2
    echo -e "  ${CYAN}Token       : ${WHITE}${token}${NC}" >&2
...
create_client_config() {
    ...
    echo "" >&2
    echo -e "  ${BOLD}═══ Client Configuration ═══════════════════════════${NC}" >&2
    echo "" >&2
    ...
    while [[ -z "$remote_addr" ]]; do
        warn "Remote address is required." >&2
...
mux_streambuffer = 65536"
    ...
    echo "" >&2
    ok "Client config saved: ${config_file}" >&2
